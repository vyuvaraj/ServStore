...
- **Next-Gen Storage Core**:
  ...
  - [ ] **HNSW Vector Indexing**: Upgrade TF-IDF to a true HNSW vector index using local ONNX embeddings for advanced semantic search queries. *(Production-grade AI-native storage — priority item.)*
...
- **Ecosystem Integration (Serv-verse)**:
  - [ ] **`/console/schema` API endpoint**: Expose table/index and bucket metadata for the ServConsole DB Inspector and Schema ORM Viewer to query.
